    ...
    k = k||(function(){
        if(typeof(localStorage)=="object"){ // update from localStorage if available
            if(localStorage.seerAPIkey){
                that.seerAPIkey=localStorage.seerAPIkey
            }else{
...
caviz.seerGet=async function(url){
    if(!this.seerAPIkey){
        console.log('seerAPIkey not found, logging in first ...')
    ...
    return (await fetch(url,{
        headers:{
            'X-SEERAPI-Key':caviz.seerAPIkey
        }
    })).json()
}
